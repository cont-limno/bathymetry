...
```{r hypsography, eval=TRUE, fig.height = 2.8}
...
gg_hypso_ct <- ggplot(data = hypso_ct) +
  ...
  ggtitle(paste0("CT", " (n=", length(unique(hypso_ct$llid)), ") lakes [1]")) + 
  theme(legend.position = c(0.84, 0.2), 
        legend.title = element_blank())
  
gg_hypso_mn <- ggplot(data = hypso_mn) +
  geom_polygon(data = polys, aes(x = x, y = y, group = id, fill = id), alpha = 0.2) +
  geom_line(aes(x = area_percent, y = depth_percent, group = llid)) +
  geom_abline(intercept = -100, slope = 1, color = "red") +
  ylim(c(100, 0)) +
  ylab("Depth (percent)") + xlab("Area (percent)") +
  ggtitle(paste0("MN", " (n=", length(unique(hypso_mn$llid)), ") lakes [4]")) +
  theme(axis.title.y = element_blank(), 
        axis.ticks.y = element_blank(), 
        axis.text.y = element_blank(),
        legend.position = c(0.72, 0.22), 
        legend.margin = margin(0, 4, 1, 1),
        legend.title = element_blank()) 

plot_grid(gg_hypso_ct + theme(legend.position = "none"), gg_hypso_mn, 
rel_widths = c(1, 0.88))

gg_hypso_ct <- gg_hypso_ct + theme(plot.title = element_blank())
# for poster
ggdraw(gg_hypso_ct, ylim = c(0, 1), clip = "off") + 
  draw_label("200 MN and CT lakes with bathymetry [1,4]", 
             y = 0.06, x = 0.22, size = 8) +
  draw_label("33 percent concave, 66 percent convex", 
             y = 0.01, x = 0.2, size = 8)
```

```{r classify, eval=TRUE}
# round the x value and test if y is greater or less than a straight-line curve
hypso_ct_classes <- hypso_ct %>%
  group_by(WBNAME) %>%
  ...
  ungroup() %>%   dplyr::filter(!is.na(shape_class)) %>% 
    dplyr::select(WBNAME, llid, shape_class)

# table(hypso_ct_classes$shape_class)

hypso_ct_w_class <- left_join(hypso_ct_classes, hypso_ct)

ggplot(data = hypso_ct_w_class) +
  geom_line(aes(x = area_percent, y = depth_percent, group = llid, 
                color = shape_class)) +
  geom_abline(intercept = -100, slope = 1) +
  ylim(c(100, 0)) +
  ylab("Depth (percent)") + xlab("Area (percent)") + 
  ggtitle(paste0("CT", " (", length(unique(hypso_ct$llid)), ") lakes")) + 
  theme(legend.position = "bottom") +
  labs(fill = "")
```

```{r class-depths, eval=TRUE}

distinct(hypso_ct_w_class, llid, .keep_all = TRUE) %>%
  dplyr::filter(shape_class != "neither") %>%
  group_by(shape_class) %>%
  summarize(depth = list(tibble::enframe(
    quantile(depth, probs = c(0.05,0.5,0.95), na.rm = TRUE)))) %>% 
  unnest() %>%
...
```{r depth-histogram, eval=TRUE}
# using Lauren's rounded (pretty) depth classes
dt2 <- read.csv("data/lagosus_depth.csv", stringsAsFactors = FALSE) %>%
  dplyr::filter(!is.na(max_depth_m)) %>%
  mutate(max_depth_m_class = smart_cut(max_depth_m, c(10, 30, 70, 200, Inf)))

table(dt2$max_depth_m_class)
round((table(dt2$max_depth_m_class) / nrow(dt2)) * 100, 2)
dplyr::filter(dt2, max_depth_m > 200)
```

```{r raster-to-profile-to-hypso, eval=FALSE, fig.height = 4}
...
# "pickerel_lake" 7096 # convex
# "long_pond" 99642 # concave

# dplyr::filter(hypso_ct, 
#               str_detect(WBNAME, c("Pickerel", "Long"))) %>%
#   distinct(llid)

rsub1 <- raster::raster("data/ct_bathy/7096.tif")
rsub2 <- raster::raster("data/ct_bathy/99642.tif")
names(rsub1) <- "layer"
names(rsub2) <- "layer"

# gg_raster <- cowplot::plot_grid( 
gg_concave_raster <- ggplot(data = as.data.frame(rsub1, xy = TRUE)) +
...
gg_convex_raster <- ggplot(data = as.data.frame(rsub2, xy = TRUE)) +
  geom_raster(aes(x = x, y = y, fill = layer)) +
  coord_quickmap() +
  scale_fill_continuous(tran = "reverse") +
...
if(!(file.exists("data/elevatr/7096.gri") & 
     file.exists("data/elevatr/99642.gri"))){
  elev <- get_elev_raster(rsub1, 13)
  raster::writeRaster(elev, paste0("data/elevatr/", 
                                   gsub("X", "", names(rsub1))
                                   ))
  
  elev <- get_elev_raster(rsub2, 13)
  raster::writeRaster(elev, paste0("data/elevatr/", 
                                   gsub("X", "", names(rsub2))
                                   ))
}
# "pickerel_lake" 7096 # convex
# "long_pond" 99642 # concave
elev1 <- raster::raster("data/elevatr/7096.gri")
elev2 <- raster::raster("data/elevatr/99642.gri")

prep_rayshader <- function(rsub, elev){
  rsub <- rsub * -1
  maxdepth <- abs(cellStats(rsub, "min"))
  
  test <- crop(elev, rsub)
  test <- test - abs(cellStats(test, "min")) # test should have a min of 0
  rsub <- rsub + abs(cellStats(rsub, "max")) # rsub should have a max of 0

  test  <- mask(test, test <= (cellStats(test, "min") + 1), maskvalue = TRUE)
  test  <- resample(test, rsub)
  test2 <- merge(rsub, test)
  # test2[is.na(test2)] <- 0

  rmat                <- matrix(test2 + maxdepth, 
                                nrow = ncol(rsub), ncol = nrow(rsub))
  rmat[rmat > maxdepth] <- NA
  list(rmat = rmat, maxdepth = maxdepth, r = test2)
}
```

```{r gg-rayshader-concave, eval=FALSE}

res_rsub1 <- prep_rayshader(rsub1, elev1)
# plot(res_rsub1$r)

sphere_shade(res_rsub1$rmat, texture = tex) %>%
  plot_3d(res_rsub1$rmat, water = TRUE, waterdepth = res_rsub1$maxdepth,
          zscale = 0.4, solidcolor = "white", solidalpha = 0.4,
          shadow = FALSE, solidlinecolor = "white", phi = 10, theta = -20)
render_snapshot("figures/concave-rayshader.png", clear = TRUE)
magick::image_read("figures/concave-rayshader.png") %>%
  magick::image_trim() %>%
  magick::image_write("figures/concave-rayshader.png")

cowplot::plot_grid(
  gg_concave_raster, 
  ggdraw() +
  draw_image("figures/concave-rayshader.png")
)
```

```{r gg-rayshader-convex, eval=FALSE}
res_rsub2 <- prep_rayshader(rsub2, elev2)

sphere_shade(res_rsub2$rmat, texture = tex) %>%
  plot_3d(res_rsub2$rmat, water = TRUE, waterdepth = res_rsub2$maxdepth,
          zscale = 0.4, solidcolor = "white", solidalpha = 0.4,
          shadow = FALSE, solidlinecolor = "white", phi = 10, theta = -20)
render_snapshot("figures/convex-rayshader.png", clear = TRUE)
magick::image_read("figures/convex-rayshader.png") %>%
  magick::image_trim() %>%
  magick::image_write("figures/convex-rayshader.png")

cowplot::plot_grid(
  gg_convex_raster, 
  ggdraw() +
  draw_image("figures/convex-rayshader.png")
)
```

```{r get-hypso, eval=FALSE, fig.height = 4}
# "pickerel_lake" 7096 # convex
# "long_pond" 99642 # concave
rsub1 <- raster::raster("data/ct_bathy/7096.tif")
rsub2 <- raster::raster("data/ct_bathy/99642.tif")
...
```{r get-hypso-normalized, eval=FALSE, fig.height = 4}
gg_radius_normal <- ggplot(data = hypso, 
                           aes(x = radius_percent, y = depth_percent)) +
    geom_point(aes(color = type)) + geom_line(aes(color = type)) +
    xlab("Radius distance (percent)") + ylab("Depth (percent)") + 
    ylim(c(100, 0)) +  theme(legend.position = "none")

gg_hypso_normal <- ggplot(data = hypso, aes(x = area_percent, y = depth_percent)) +
  geom_point(aes(color = type)) + geom_line(aes(color = type)) +
  xlab("Area > depth (percent)") + ylab("Depth (percent)") +
    ylim(c(100, 0))

cowplot::plot_grid(
  gg_radius_normal, gg_hypso_normal,
rel_widths = c(0.6, 1)
)
...
gg_radius_convex <- 
  ggplot(data = dplyr::filter(hypso, type == "convex"), 
         aes(x = radius_percent, y = depth)) +
    geom_point() + geom_line() +
    xlab("Radius distance (proportion)") + ylab("Depth") +
    scale_y_reverse()
  
gg_hypso_convex <- ggplot(data = dplyr::filter(hypso, type == "convex"), 
                           aes(x = area_percent, y = depth)) +
    geom_point() + geom_line() +
    xlab("Area > depth (percent)") + ylab("Depth") +
  scale_y_reverse()

cowplot::plot_grid(
  gg_convex_raster + theme(plot.title = element_blank()), 
  ggdraw() +
  draw_image("figures/convex-rayshader.png"),
  gg_radius_convex, 
  gg_hypso_convex, 
...
```{r compare-slopes, eval=FALSE, fig.height = 4}
