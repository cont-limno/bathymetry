...
hypso1$test <- scales::rescale(c(diff(hypso1$n_cs), 0), to = c(0, 100))
...
cowplot::plot_grid(
ggplot(data = hypso1, aes(x = test, y = depth_percent)) +
  geom_point(aes(y = rev(depth_percent))) + xlab("Area (percent)") + ylab("Depth (percent)"), 
ggplot(data = hypso1, aes(x = area_percent, y = depth_percent)) +
  geom_point() + xlab("Area > depth (percent)") + ylab("Depth (percent)")
)
