...
```{r 01_contrasts_depth, echo=FALSE, message=FALSE, eval=TRUE, warning=FALSE, results='hide', fig.height = 6}
...
read.csv(
  "../data/lagosus_depth.csv", stringsAsFactors = FALSE) %>% 
   dplyr::filter(!is.na(lake_maxdepth_m)) %>%
   group_by(programtype_depth, programlink_depth) %>% 
   tally() %>% 
   ungroup() %>% group_by(programtype_depth) %>%
   tally() %>%
   ggplot(aes(x = programtype_depth, y = n)) + 
   geom_col() + 
   geom_text(aes(label = n), vjust = -0.5) + 
