...
gg_area_shape_class <- dt %>%
  dplyr::filter(shape_class != "neither") %>%
  group_by(shape_class) %>%
  summarize(lake_waterarea_ha = list(tibble::enframe(
    quantile(lake_waterarea_ha, probs = c(0.05,0.5,0.95), na.rm = TRUE)))) %>% 
  unnest() %>%
  tidyr::spread(name, value) %>%
  ggplot() +
geom_pointrange(aes(x = shape_class, y = `50%`, 
...
gg_slope_shape_class <- dt %>%
  dplyr::filter(shape_class != "neither") %>%
  group_by(shape_class) %>%
  summarize(inlake_slope = list(tibble::enframe(
    quantile(inlake_slope, probs = c(0.05,0.5,0.95), na.rm = TRUE)))) %>% 
  unnest() %>%
  tidyr::spread(name, value) %>%
  ggplot() +
geom_pointrange(aes(x = shape_class, y = `50%`, 
                      ymin = `5%`, ymax = `95%`)) +
  ylab("Inlake slope (m/m)") + xlab("")
```
...
```{r 01_contrasts, echo=FALSE, message=FALSE, eval=TRUE, warning=FALSE, results='hide', fig.height = 4}
...
gg_shape <- cowplot::plot_grid(
  ...
  gg_area_shape_class + ggtitle("Inlake slope (m/m)") + theme_opts, 
  gg_area_shape_class + ggtitle("Deepest point distance (m)") + theme_opts, 
  nrow = 1)
...
```{r 01_programs_tally, echo=FALSE, message=FALSE, eval=TRUE, warning=FALSE, results='hide'}
# number of programs in each type
read.csv(
  "../data/lagosus_depth.csv", stringsAsFactors = FALSE) %>% 
   dplyr::filter(!is.na(lake_maxdepth_m)) %>%
   group_by(sourcetype_depth, sourceurl_depth) %>% 
   tally() %>% 
