...
gg_slope_shape_class <- dt %>%
  dplyr::filter(shape_class != "neither") %>%  
  contrast_plot(grp = "shape_class", varb = "inlake_slope", 
...
gg_slope_reservoir <- dt %>%
  dplyr::filter(!is.na(reservoir_class)) %>%  
  contrast_plot(grp = "reservoir_class", varb = "inlake_slope", 
                label = "In-lake slope (m/m)", plot_type = "pointrange")
```
...
gg_shorelinedev_shape_class <- dt %>%
  dplyr::filter(shape_class != "neither") %>%  
  contrast_plot(grp = "shape_class", varb = "lake_shorelinedevfactor_nounits", 
...
```{r gg_effort, echo=FALSE, message=FALSE, eval=TRUE, warning=FALSE, results='hide', fig.height = 3.5}
...
test <- apply(st_distance(LAGOSUS::coordinatize(lg, crs = LAGOSUSgis:::albers_conic()),
            urbanareas), 1, function(x) x[which.min(x)])
```
