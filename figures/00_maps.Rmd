...
gg_states <- 
  ...
  # geom_sf(data = hu4, fill = "grey") +
    ...
    geom_point_rast(data = dplyr::filter(dt_notsf, lake_maxdepth_m < 10), 
            aes(x = X, y = Y, color = maxdepth_class), size = 0.3, alpha = 0.8) +
    geom_point_rast(data = dplyr::filter(dt_notsf, lake_maxdepth_m > 10), 
            aes(x = X, y = Y, color = maxdepth_class), size = 0.3, alpha = 0.8) +
...
dt_bathy <- mutate(dt_bathy, maxdepth_class = droplevels(dt_bathy$maxdepth_class)) 
...
gg_bathy <- 
  ggplot() + 
    geom_sf(data = st_as_sfc(st_bbox(dt_bathy)), alpha = 1, size = 0.6, 
          color = "black", fill = "#1D9DCF") +
    geom_sf(data = st_crop(states_all, st_as_sfc(st_bbox(dt_bathy))), fill = "gray", color = "black", size = 0.3) +
    geom_sf(data = st_crop(states_focal, st_as_sfc(st_bbox(dt_bathy))), fill = "white", color = "black", size = 0.4) + 
...
is_colocated <- read.csv("../data/lagosus_depth.csv", stringsAsFactors = FALSE) %>%
  dplyr::filter(
...
gg_shapeclass <- ggplot() + 
    ...
    geom_sf(data = st_crop(states_all, st_as_sfc(st_bbox(dt_bathy))), fill = "gray", color = "black", size = 0.3) +
    geom_sf(data = st_crop(states_focal, st_as_sfc(st_bbox(dt_bathy))), fill = "white", color = "black", size = 0.4) + 
    geom_sf(data = dplyr::filter(dt, shape_class == "convex"), aes(color = shape_class),
                    size = 0.03, alpha = 0.3) +
    geom_sf(data = dplyr::filter(dt, shape_class != "convex"), aes(color = shape_class),
                    size = 0.07, alpha = 0.8) +
...
```{r 00_map_bathy, echo=FALSE, message=FALSE, eval=TRUE, warning=FALSE, results='hide'}
cowplot::plot_grid(gg_bathy, 
                   gg_shapeclass, ncol = 1)
```
