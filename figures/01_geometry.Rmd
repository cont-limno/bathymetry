...
```{r slope-compare, echo=FALSE, message=FALSE, eval=TRUE, warning=FALSE, results='hide'}
...
cowplot::plot_grid(
ggplot(data = test) + 
    geom_point(aes(slope_mean, inlake_slope.y, color = reservoir_class)) +
  xlab("Buffer slope (mean)") +
  ylab("In-lake slope") +
  labs(color = "") + 
  theme(axis.title.x = element_blank()),
ggplot(data = test) + 
    geom_point(aes(slope_mean, inlake_slope.y, color = shape_class)) +
  xlab("Buffer slope (mean)") +
  ylab("In-lake slope") +
  labs(color = ""), 
nrow = 2 
