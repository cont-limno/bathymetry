...
gg_scatter_shape <- st_drop_geometry(dt) %>%
  ...
  ggplot() + 
  geom_point(aes(x = dist_deepest, y = dist_viscenter, color = shape_class)) +
  # geom_abline(aes(intercept = 0, slope = 1)) +
  labs(color = "") +
  xlab("Deepest point distance (m)") +
  ylab("Visual center distance (m)")

gg_scatter_reservoir <- ggplot(data = dplyr::filter(
  st_drop_geometry(dt), !is.na(reservoir_class))) + 
...
```{r 01_geometry_grid, echo=FALSE, message=FALSE, eval=TRUE, warning=FALSE, results='hide', fig.width=5}
...
cowplot::plot_grid(
  cowplot::plot_grid(gg_scatter_shape + 
                     ...
                     gg_slope_reservoir + 
                       theme(axis.title.y = element_blank()) +
                       theme_opts, 
                     rel_widths = c(1, 0.93, 1, 1)), 
  legend_panel, rel_widths = c(1, 0.2)
)
```
