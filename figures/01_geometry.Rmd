...
gg_slope_shape <- ggplot(data = test) + 
  geom_point(aes(slope_mean, inlake_slope.y, 
                             color = shape_class), size = 1) +
  geom_smooth(aes(slope_mean, inlake_slope.y, 
...
gg_dist_base <- st_drop_geometry(dt) %>%
  ...
  annotate("text", x = 2550, y = 300, label = "R^2 == 0.80",
                                parse = TRUE, size = text_size) +
  annotate("text", x = 2100, y = 150, 
           label = get_formula(
...
dt <- left_join(dt, st_drop_geometry(bp), 
                by = "llid") %>%
  dplyr::filter(!is.na(inlake_slope_mean)) %>%
  dplyr::filter(inlake_slope.y < 0.4)
lgus_pred <- read.csv("../data/lagosus_depth_predictors.csv", stringsAsFactors = FALSE) %>%
  mutate(lagoslakeid = as.character(lagoslakeid))
test <- left_join(dt, lgus_pred, by = c("llid" = "lagoslakeid")) %>% 
  dplyr::filter(!is.na(shape_class)) %>%
...
gg_slope_base <- ggplot(data = test) + 
  geom_point(aes(slope_mean, inlake_slope.y), size = 1) +
  geom_smooth(aes(slope_mean, inlake_slope.y), method = "lm", se = FALSE) +
  geom_rect(xmin = 0.28, xmax = 0.42, ymin = 0, ymax = 0.05,
  alpha = 1, fill = "white") +
  annotate("text", x = 0.34, y = 0.03, label = "R^2 == 0.17",
                                parse = TRUE, size = text_size) +
  annotate("text", x = 0.34, y = 0.01, 
           label = get_formula(
             lm(inlake_slope.y ~ slope_mean, data = test)), 
           size = text_size) +
  # geom_abline(slope = 1, intercept = 0) +
  xlab("Nearshore slope (mean)") +
  ylab("In-lake slope") +
...
gg_area <- ggplot(data = dt) +
   geom_point(aes(x = slope_mean, y = inlake_slope, color = x_bins)) +
   geom_smooth(aes(slope_mean, inlake_slope, color = x_bins),
               method = "lm", se = FALSE) + 
