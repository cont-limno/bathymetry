...
geom_pointrange(aes(x = reservoir_class, y = `50%`, 
                      ymin = `5%`, ymax = `95%`)) +
  ylab("Distance from shore to deepest point (m)") + xlab("")
...
```{r slope-compare, echo=FALSE, message=FALSE, eval=FALSE, warning=FALSE, results='hide'}
...
plot(dt$inlake_slope_mean, dt$slope_online_mean)
plot(dt$inlake_slope_mean, dt$slope_mean)
plot(dt$inlake_slope_mean, dt$slope_mean_ne)
plot(dt$slope_mean, dt$slope_mean_ne)
dt$res_elev_1
plot(dt$inlake_slope_median, dt$slope_online_median)

plot(dt$inlake_slope, dt$slope_mean_ne)
